...
CREATE PROCEDURE update_cart_item_quantity (
...
BEGIN
    SELECT * FROM v_product_catalog WHERE product_id = p_product_id;
END //
...
BEGIN
    ...
    WHERE product_name LIKE CONCAT('%', p_search_term, '%')
       ...
       OR category_name LIKE CONCAT('%', p_search_term, '%')
    LIMIT p_page_size OFFSET (p_page_number - 1) * p_page_size;
END //
...
-- Add a Product with Diamond Details
CREATE PROCEDURE add_product_with_diamond (
    IN p_company_id INT,
    IN p_category_id INT,
    IN p_name VARCHAR(255),
...
-- =====================================
-- 3. User & Admin Management Procedures
-- =====================================

-- Register a New User and Create a Default Cart
CREATE PROCEDURE register_new_user (
    IN p_first_name VARCHAR(100),
    IN p_last_name VARCHAR(100),
    IN p_email VARCHAR(255),
...
-- Add a new company
CREATE PROCEDURE add_new_company (
    IN p_name VARCHAR(255),
...
BEGIN
    INSERT INTO companies (name, logo_url, description, email, phone, address, gst_number)
    VALUES (p_name, p_logo_url, p_description, p_email, p_phone, p_address, p_gst_number);
END //


-- =====================================
-- 4. Order & Feedback Procedures
-- =====================================
...
CREATE PROCEDURE get_all_products_by_company (
    IN p_company_id INT
)
BEGIN
    SELECT * FROM v_product_catalog WHERE company_id = p_company_id;
END //
...
-- Get a list of all tracking updates for an order
CREATE PROCEDURE get_order_tracking_updates (
    IN p_order_id INT
)
BEGIN
    SELECT * FROM order_tracking WHERE order_id = p_order_id ORDER BY created_at DESC;
END //
...
CREATE PROCEDURE add_product_review (
    IN p_product_id INT,
    IN p_user_id INT,
    IN p_rating TINYINT,
    IN p_comment TEXT
...
CREATE PROCEDURE get_company_orders (
    IN p_company_id INT
)
BEGIN
    SELECT
